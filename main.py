...
class BookingCreate(BaseModel):
    ...
    def _guests_within_package(self):
        ...
        # A table booking should carry a table_id; solo entry should not.
...
def get_tables():
    ...
    # Confirmed = permanently taken (no time filter).
    confirmed = db().table("bookings").select("table_id") \
        .eq("status", "confirmed") \
        .not_.is_("table_id", "null") \
        .execute()

    # Pending / verifying = soft holds that expire after 15 min.
...
def create_booking(payload: BookingCreate):
    if payload.table_id:
        ...
        # Blocked if the table is already confirmed (permanent)...
        confirmed = db().table("bookings").select("id") \
            .eq("table_id", payload.table_id) \
            .eq("status", "confirmed") \
            .execute()

        # ...or currently held by an unexpired pending/verifying booking.
